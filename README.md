...
This is a fork of VitaliiBlagodir's DatePicker plugin, which fixes an android layout bug.
...
```bash
cordova plugin add flyacts-cordova-plugin-datepicker
```

- Local development workflow using [PhoneGap CLI](http://phonegap.com/install/)

```bash
phonegap local plugin add flyacts-cordova-plugin-datepicker
```

- Cloud-based development workflow using [PhoneGap Build](http://build.phonegap.com)

```bash
<gap:plugin name="flyacts-cordova-plugin-datepicker" source="npm" />
```
